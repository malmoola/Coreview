...
const pkg = {
  ...
  doc:{canvas:{grid:true,snap:true,zoom:1,x:0,y:0},
    ...
    probes:[]}};
await p.addInitScript((x)=>localStorage.setItem("coreview.projects.v1",JSON.stringify({perf:x})),pkg);
...
console.log("LS doc nodes now:",await p.evaluate(()=>JSON.parse(localStorage.getItem("coreview.projects.v1")).perf.doc.nodes.length));
await b.close();
